...
- `[24]` — `accept_prob`: вероятность того, что соперник примет текущее предложение удвоения (`sigmoid`).
...
- `accept_prob = probs[24]` для оценки ответа соперника при решении, предлагать ли удвоение;
